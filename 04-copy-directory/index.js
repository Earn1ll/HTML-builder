...
const copyPaste = (async function () {
  try {
    fsPromises
      .rm(copyDirPath, { recursive: true, force: true })
      .then(async () => {
        const createDir = await fsPromises.mkdir(copyDirPath, {
          recursive: true,
        });
        await copyDir(pathFrom).then(
          console.log(
            "Здравствуйте, папка files скопирована в папку files-copy"
          )
        );
      });
  } catch (err) {
...
const copyDir = async function (pathFrom) {
  try {
    ...
    files.forEach((file) => {
      if (file.isDirectory()) {
        fsPromises.mkdir(
          path.join(pathFrom.replace("files", "files-copy"), file.name),
          { recursive: true }
        );
